...
本文档定义抽取结果、任务状态、统一错误码及关联数据结构的字段约定。

---

## 目录

1. [抽取结果 JSON](#1-抽取结果-json)
2. [任务状态枚举](#2-任务状态枚举)
3. [统一错误码](#3-统一错误码)
4. [API 错误响应格式](#4-api-错误响应格式)
5. [执行日志最小字段](#5-执行日志最小字段)
6. [填表报告结构](#6-填表报告结构)
7. [版本与变更说明](#7-版本与变更说明)
...
同目录已提供 **`extraction-result.schema.json`**（JSON Schema Draft 2020-12），可供服务侧校验或与 CI 集成；变更时请与 `schema_version` 同步升级。
...
- 成功可统一为：`{ "success": true, "data": { ... } }`；错误体建议仍用上述结构。
...
填表任务完成后，除输出文件外，建议产出**结构化报告**，便于入库与质检。
...
若无对象存储，可用 `local_path` 替代 `storage_key`，须在部署说明中约定路径规则。

---

## 7. 版本与变更说明

- 破坏性变更须升 `schema_version` 或契约主版本（如 v1 → v2）。
- 同步更新本文档与同目录 `extraction-result.schema.json`，并更新仓库变更记录或 tag。
